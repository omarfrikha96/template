...
package salesforce.getonedb_0_1;
 ...
 * @status
 */
public class GetOneDb implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "GetOneDb.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(GetOneDb.class);
	...
	private final String jobName = "GetOneDb";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(GetOneDb.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class RestServiceProviderImpl4TalendJob {
		...
		private final GetOneDb job;

		public RestServiceProviderImpl4TalendJob(GetOneDb job) {
			this.job = job;
		}

		private void populateRequestWithJobContext(java.util.Map<String, Object> requestGlobalMap, GetOneDb job) {
	...
	class Thread4RestServiceProviderEndpoint extends Thread {
		...
		public Thread4RestServiceProviderEndpoint(GetOneDb job, String endpointUrl) {
			this.job = job;
	...
	public static class ContextBean {
		static String evaluate(String context, String contextExpression)
			...
			java.io.InputStream inContext = GetOneDb.class.getClassLoader()
					.getResourceAsStream("salesforce/getonedb_0_1/contexts/" + context + ".properties");
			if (inContext == null) {
				inContext = GetOneDb.class.getClassLoader()
	...
	public static class row7Struct implements routines.system.IPersistableRow<row7Struct> {
		final static byte[] commonByteArrayLock_SALESFORCE_GetOneDb = new byte[0];
		static byte[] commonByteArray_SALESFORCE_GetOneDb = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_SALESFORCE_GetOneDb.length) {
					if (length < 1024 && commonByteArray_SALESFORCE_GetOneDb.length == 0) {
						commonByteArray_SALESFORCE_GetOneDb = new byte[1024];
					} else {
						commonByteArray_SALESFORCE_GetOneDb = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_SALESFORCE_GetOneDb, 0, length);
				strReturn = new String(commonByteArray_SALESFORCE_GetOneDb, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_SALESFORCE_GetOneDb, 0, length);
				strReturn = new String(commonByteArray_SALESFORCE_GetOneDb, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_SALESFORCE_GetOneDb) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tDBConnection_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBConnection_1.append("PASS" + " = " + String.valueOf(
									"enc:routine.encryption.key.v1:fUpW2wR35wwyNQhe3a17JDXU3psrdt2B/77jGsdrfs4=")
									.substring(0, 4) + "...");
							log4jParamters_tDBConnection_1.append(" | ");
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:+BLGt5yDCMAnoJE+F63z+FF971OrlMnHYBTuf+kQjpU=");
...
//    	this.treeNodeAPI = treeNodeAPI;
					...
					// input table.
					void generateElements(boolean isInnerJoin, row6Struct row6,
...
// build root xml tree 
						...
						valueMap.put("loop_0", row6.CD_PRODUIT);
						...
						valueMap.put("loop_1", row6.NOM_PRODUIT);
						...
						valueMap.put("loop_2", row6.PRIX_ACHAT_PRODUIT);
						...
						valueMap.put("loop_3", row6.PRIX_VENTE_PRODUIT);
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						if (ope_tFilterRow_1.getMatchFlag()) {
							...
							row6.CD_PRODUIT = row1.CD_PRODUIT;
							row6.NOM_PRODUIT = row1.NOM_PRODUIT;
							row6.PRIX_ACHAT_PRODUIT = row1.PRIX_ACHAT_PRODUIT;
							row6.PRIX_VENTE_PRODUIT = row1.PRIX_VENTE_PRODUIT;
...
//construct the resultset
								...
								gen_Doc_oneProduct_tXMLMap_1_TXMLMAP_OUT
										.generateElements(rejectedDocInnerJoin_tXMLMap_1_TXMLMAP_OUT, row6, Var);
...
// Start of branch "header"
	...
	public static void main(String[] args) {
		final GetOneDb GetOneDbClass = new GetOneDb();

		int exitCode = GetOneDbClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'GetOneDb' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'GetOneDb' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = GetOneDb.class.getClassLoader()
					.getResourceAsStream("salesforce/getonedb_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = GetOneDb.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : GetOneDb");
		}
...
/************************************************************************************************
 * 314993 characters generated by Talend Open Studio for ESB on the 15 août 2024
 * à 2:32:54 PM CET
 ************************************************************************************************/