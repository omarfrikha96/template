...
package salesforce.copy_of_getallrequest_b_0_1;
...
/**
 * Job: Copy_of_GetAllRequest_b Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class Copy_of_GetAllRequest_b implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "Copy_of_GetAllRequest_b.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(Copy_of_GetAllRequest_b.class);
	...
	private final String jobName = "Copy_of_GetAllRequest_b";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Copy_of_GetAllRequest_b.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Copy_of_GetAllRequest_b.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class RestServiceProviderImpl4TalendJob {
		...
		private final Copy_of_GetAllRequest_b job;

		public RestServiceProviderImpl4TalendJob(Copy_of_GetAllRequest_b job) {
			this.job = job;
		}

		private void populateRequestWithJobContext(java.util.Map<String, Object> requestGlobalMap,
				Copy_of_GetAllRequest_b job) {
	...
	class Thread4RestServiceProviderEndpoint extends Thread {
		...
		public Thread4RestServiceProviderEndpoint(Copy_of_GetAllRequest_b job, String endpointUrl) {
			this.job = job;
	...
	public static class ContextBean {
		static String evaluate(String context, String contextExpression)
			...
			java.io.InputStream inContext = Copy_of_GetAllRequest_b.class.getClassLoader()
					.getResourceAsStream("salesforce/copy_of_getallrequest_b_0_1/contexts/" + context + ".properties");
			if (inContext == null) {
				inContext = Copy_of_GetAllRequest_b.class.getClassLoader()
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tDBConnection_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBConnection_1.append("PASS" + " = " + String.valueOf(
									"enc:routine.encryption.key.v1:h7uD8zmGu28PSQJ4usQDSNxmRF3oMK7DfyHBWJDSmmc=")
									.substring(0, 4) + "...");
							log4jParamters_tDBConnection_1.append(" | ");
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:Hvk+kVFcqSlhC2etiUc3kt1RdBX3+jabAbLbGm0/0uE=");
	...
	public static class ressssStruct implements routines.system.IPersistableRow<ressssStruct> {
		final static byte[] commonByteArrayLock_SALESFORCE_Copy_of_GetAllRequest_b = new byte[0];
		static byte[] commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b = new byte[0];
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_SALESFORCE_Copy_of_GetAllRequest_b) {
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b.length) {
					if (length < 1024 && commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b.length == 0) {
						commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b = new byte[1024];
					} else {
						commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b, 0, length);
				strReturn = new String(commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b, 0, length);
				strReturn = new String(commonByteArray_SALESFORCE_Copy_of_GetAllRequest_b, 0, length, utf8Charset);
			}
			return strReturn;
		}
	...
	public void tSetGlobalVar_3Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tSetGlobalVar_3 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tSetGlobalVar_3.append("VARIABLES" + " = " + "[{VALUE=" + ("401") + ", KEY="
									+ ("\"httpCode\"") + "}, {VALUE=" + ("\"INTERNAL\"") + ", KEY="
									+ ("\"cause_system\"") + "}, {VALUE=" + ("\"401\"") + ", KEY="
									+ ("\"cause_errorCode\"") + "}, {VALUE="
									+ ("\"Check request parameters : \" + ((String)globalMap.get(\"s_authentication\"))")
									+ ", KEY=" + ("\"cause_message\"") + "}]");
				...
				globalMap.put("cause_system", "INTERNAL");
				globalMap.put("cause_errorCode", "401");
				globalMap.put("cause_message",
						"Check request parameters : " + ((String) globalMap.get("s_authentication")));
            ...
            if (log.isDebugEnabled()) {
                class BytesLimit65535_tSetGlobalVar_1{
                    ...
                    log4jParamters_tSetGlobalVar_1.append("Parameters:");
                            log4jParamters_tSetGlobalVar_1.append("VARIABLES" + " = " + "[{VALUE="+("((TokenValidatorService)globalMap.get(\"tokenValidatorService\")).validateToken(header.Authorization)")+", KEY="+("\"is_valid_token\"")+"}, {VALUE="+("0")+", KEY="+("\"i_stepSequence\"")+"}, {VALUE="+("JacquemusUtils.isBlank(header.correlationId) ? StringHandling.SUBSTR((java.util.UUID.randomUUID()).toString(), 10, 14) : header.correlationId")+", KEY="+("\"s_correlationId\"")+"}]");
					...
					if (matchedFlow_tRESTRequest_1.equals("header")) {
						header = new headerStruct();
								if (params_tRESTRequest_1.containsKey("HEADER:Authorization:id_String")) {
										...
										header.Authorization = (String) params_tRESTRequest_1.get("HEADER:Authorization:id_String");
									
								}
								if (params_tRESTRequest_1.containsKey("PATH:correlationId:id_String")) {
									
										
										header.correlationId = (String) params_tRESTRequest_1.get("PATH:correlationId:id_String");
...
globalMap.put("is_valid_token", ((TokenValidatorService)globalMap.get("tokenValidatorService")).validateToken(header.Authorization));
globalMap.put("i_stepSequence", 0);
globalMap.put("s_correlationId", JacquemusUtils.isBlank(header.correlationId) ? StringHandling.SUBSTR((java.util.UUID.randomUUID()).toString(), 10, 14) : header.correlationId);
 ...
 */
	...
	public static void main(String[] args) {
		final Copy_of_GetAllRequest_b Copy_of_GetAllRequest_bClass = new Copy_of_GetAllRequest_b();

		int exitCode = Copy_of_GetAllRequest_bClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'Copy_of_GetAllRequest_b' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'Copy_of_GetAllRequest_b' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = Copy_of_GetAllRequest_b.class.getClassLoader().getResourceAsStream(
					"salesforce/copy_of_getallrequest_b_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Copy_of_GetAllRequest_b.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : Copy_of_GetAllRequest_b");
		}
...
/************************************************************************************************
 * 179802 characters generated by Talend Open Studio for ESB on the 9 août 2024
 * à 1:46:17 PM CET
 ************************************************************************************************/