...
package salesforce.postrequest_0_1;
 ...
 * @status
 */
public class PostRequest implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "PostRequest.log");
	}
	...
	private final String jobName = "PostRequest";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(PostRequest.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class RestServiceProviderImpl4TalendJob {
		...
		private final PostRequest job;

		public RestServiceProviderImpl4TalendJob(PostRequest job) {
			this.job = job;
		}

		private void populateRequestWithJobContext(java.util.Map<String, Object> requestGlobalMap, PostRequest job) {
	...
	class Thread4RestServiceProviderEndpoint extends Thread {
		...
		public Thread4RestServiceProviderEndpoint(PostRequest job, String endpointUrl) {
			this.job = job;
	...
	public static class ContextBean {
		static String evaluate(String context, String contextExpression)
			...
			java.io.InputStream inContext = PostRequest.class.getClassLoader()
					.getResourceAsStream("salesforce/postrequest_0_1/contexts/" + context + ".properties");
			if (inContext == null) {
				inContext = PostRequest.class.getClassLoader()
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tDBConnection_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBConnection_1.append("PASS" + " = " + String.valueOf(
									"enc:routine.encryption.key.v1:LNCGcu2B4uY6ZCsyfuTrKpeMFtyMqymq+E9CyKIb4qI=")
									.substring(0, 4) + "...");
							log4jParamters_tDBConnection_1.append(" | ");
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:gR8mITdiLw/gGMyaWcZWyMzmUvnbbEnq3gA6ZlAhx+I=");
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_SALESFORCE_PostRequest = new byte[0];
		static byte[] commonByteArray_SALESFORCE_PostRequest = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_SALESFORCE_PostRequest.length) {
					if (length < 1024 && commonByteArray_SALESFORCE_PostRequest.length == 0) {
						commonByteArray_SALESFORCE_PostRequest = new byte[1024];
					} else {
						commonByteArray_SALESFORCE_PostRequest = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_SALESFORCE_PostRequest, 0, length);
				strReturn = new String(commonByteArray_SALESFORCE_PostRequest, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_SALESFORCE_PostRequest, 0, length);
				strReturn = new String(commonByteArray_SALESFORCE_PostRequest, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_SALESFORCE_PostRequest) {
	...
	public static class addProductStruct implements routines.system.IPersistableRow<addProductStruct> {
		final static byte[] commonByteArrayLock_SALESFORCE_PostRequest = new byte[0];
		static byte[] commonByteArray_SALESFORCE_PostRequest = new byte[0];
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final addProductStruct other = (addProductStruct) obj;
		...
		public void copyDataTo(addProductStruct other) {
		...
		public void copyKeysDataTo(addProductStruct other) {
		 ...
		 * Compare keys
		 */
		public int compareTo(addProductStruct other) {
	...
	public void tFixedFlowInput_5Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				addProductStruct addProduct = new addProductStruct();
...
// # Outputs initialization
				addProductStruct addProduct_tmp = new addProductStruct();
				addProductStruct addProduct_save = null;
//the aggregate variable
				addProductStruct addProduct_aggregate = null;
				int count_addProduct_tXMLMap_2_TXMLMAP_OUT = 0;
...
// ###############################
				...
				for (int i_tFixedFlowInput_5 = 0; i_tFixedFlowInput_5 < 1; i_tFixedFlowInput_5++) {
					...
					docToFlat_tXMLMap_2_TXMLMAP_OUT.setOriginalLoop("/products/product");
					docToFlat_tXMLMap_2_TXMLMAP_OUT.setIsOptional(false);
					if (row4.body == null || row4.body.getDocument() == null) {
						throw new RuntimeException("row4.body can't be empty");
					}
					org.dom4j.Document doc_tXMLMap_2_TXMLMAP_OUT = row4.body.getDocument();
					...
					absolutePathMappings_tXMLMap_2_TXMLMAP_OUT[0] = "row4.body:/products/product/PRIX_VENTE_PRODUIT";
					relativePathMappings_tXMLMap_2_TXMLMAP_OUT[0] = "PRIX_VENTE_PRODUIT";

					absolutePathMappings_tXMLMap_2_TXMLMAP_OUT[1] = "row4.body:/products/product/CD_PRODUIT";
					relativePathMappings_tXMLMap_2_TXMLMAP_OUT[1] = "CD_PRODUIT";

					absolutePathMappings_tXMLMap_2_TXMLMAP_OUT[2] = "row4.body:/products/product/PRIX_ACHAT_PRODUIT";
					relativePathMappings_tXMLMap_2_TXMLMAP_OUT[2] = "PRIX_ACHAT_PRODUIT";

					absolutePathMappings_tXMLMap_2_TXMLMAP_OUT[3] = "row4.body:/products/product/NOM_PRODUIT";
					relativePathMappings_tXMLMap_2_TXMLMAP_OUT[3] = "NOM_PRODUIT";
...
// # Output table : 'addProduct'
							count_addProduct_tXMLMap_2_TXMLMAP_OUT++;

							addProduct_tmp = new addProductStruct();
							addProduct_tmp.CD_PRODUIT = treeNodeAPI_tXMLMap_2_TXMLMAP_OUT
									.get_String("row4.body:/products/product/CD_PRODUIT");
							addProduct_tmp.NOM_PRODUIT = treeNodeAPI_tXMLMap_2_TXMLMAP_OUT
									.get_String("row4.body:/products/product/NOM_PRODUIT");
							addProduct_tmp.PRIX_ACHAT_PRODUIT = treeNodeAPI_tXMLMap_2_TXMLMAP_OUT
									.get_BigDecimal("row4.body:/products/product/PRIX_ACHAT_PRODUIT");
							addProduct_tmp.PRIX_VENTE_PRODUIT = treeNodeAPI_tXMLMap_2_TXMLMAP_OUT
									.get_BigDecimal("row4.body:/products/product/PRIX_VENTE_PRODUIT");
							allOutsForAggregate_tXMLMap_2.add(addProduct_tmp);

							log.debug("tXMLMap_2 - Outputting the record " + count_addProduct_tXMLMap_2_TXMLMAP_OUT
									+ " of the output table 'addProduct'.");
...
// ###############################
				...
				log.debug("tXMLMap_2 - Written records count in the table 'addProduct': "
						+ count_addProduct_tXMLMap_2_TXMLMAP_OUT + ".");
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "addProduct");
				}
				...
				for (Object row_out_tXMLMap_2_TXMLMAP_IN : outs_tXMLMap_2) {// TD512
					...
					if (row_out_tXMLMap_2_TXMLMAP_IN != null
							&& row_out_tXMLMap_2_TXMLMAP_IN instanceof addProductStruct) {
						addProduct = (addProductStruct) row_out_tXMLMap_2_TXMLMAP_IN;
					}
...
// Start of branch "addProduct"
					if (addProduct != null) {
						...
						if (log.isTraceEnabled()) {
							log.trace("addProduct - " + (addProduct == null ? "" : addProduct.toLogString()));
						}

						whetherReject_tDBOutput_1 = false;
						if (addProduct.CD_PRODUIT == null) {
							pstmt_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
						} else {
							pstmt_tDBOutput_1.setString(1, addProduct.CD_PRODUIT);
						}

						if (addProduct.NOM_PRODUIT == null) {
							pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
						} else {
							pstmt_tDBOutput_1.setString(2, addProduct.NOM_PRODUIT);
						}

						pstmt_tDBOutput_1.setBigDecimal(3, addProduct.PRIX_ACHAT_PRODUIT);

						pstmt_tDBOutput_1.setBigDecimal(4, addProduct.PRIX_VENTE_PRODUIT);
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "addProduct");
				}
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				for (int i_tFixedFlowInput_4 = 0; i_tFixedFlowInput_4 < 1; i_tFixedFlowInput_4++) {
					...
					if (null == outputStream_tRESTResponse_4 && responseAlreadySent_tRESTResponse_4) {
						...
						Integer restProviderStatusCode_tRESTResponse_4 = 400;

						Object restProviderResponse_tRESTResponse_4 = null;
						restProviderResponse_tRESTResponse_4 = row5.body;
...
// Start of branch "header"
	...
	public static void main(String[] args) {
		final PostRequest PostRequestClass = new PostRequest();

		int exitCode = PostRequestClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'PostRequest' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'PostRequest' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = PostRequest.class.getClassLoader()
					.getResourceAsStream("salesforce/postrequest_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = PostRequest.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : PostRequest");
		}
...
/************************************************************************************************
 * 211220 characters generated by Talend Open Studio for ESB on the 7 août 2024
 * à 2:57:47 PM CET
 ************************************************************************************************/